...
# generate the files and analysis ----
  ...
  if (LoadData_raw)
  {
    ...
    if (save.files){
      save (Tissues7.Raw,file = '~/Robj/Tissues.raw.Robj')
    }
  } 
## Seurat Analysis of full Tissue dataset ----
  if (Tissues7_analysis) 
  { 
    load (file = '~/Robj/Tissues.raw.Robj')
    ...
    # assign cluster ID
    clusterNames<- read_excel("~/SupplementaryData1.xlsx", 
                              sheet = 'DataS8') 
    ...
    if (save.files){
      save (Tissues7, file = '~/Robj/Tissues.Robj')
    }
  } 
...
## Seurat Analysis of "Muscle/Gastroderm" dataset ----
  if (Muscle7_analysis)
  {
   ...
   annoy.metric = 'cosine' )
    ...
    if (save.files){
      save (Muscle7, file = '~/Robj/muscle.Robj') 
    }
  }
...
## generate nem64.dataset and Seurat Analysis ----
  ...
  {  
    ...
    if (save.files){
      save (nem64,file = '~/RObj/nem64.Robj')
    }  
  }
  ...
  ### Transfer labels from wildtype dataset ----
    ...
    if (save.files){
      save(nem64.endo,file='~/RObj/nem64.gastrodermis.RObj')
    }
  }
  ...
  nem64.Muscle7 = data1
  nem64.Muscle7$IDs = nem64.Muscle7@active.ident
  if (save.files){
    save(nem64.Muscle7,file = '~/RObj/nem64.muscle7.Robj')
  }
  }
